...
$Engine = Join-Path $Root 'fps-tune.ps1'
...
function Update-ThemeControl {
    ...
    if ($Control -is [FpsTuneGui.RoundedButton]) {
    ...
    elseif ($Control -is [FpsTuneGui.RoundedPanel]) {
        $tag = [string]$Control.Tag
        if ($tag -eq 'field') {
...
function New-RoundedButton {
    ...
    $b = New-Object FpsTuneGui.RoundedButton
    $b.Text = $Text
...
function New-Card {
    ...
    $card = New-Object FpsTuneGui.RoundedPanel
    $card.Dock = 'Fill'
...
function New-FieldTextBox {
    ...
    $field = New-Object FpsTuneGui.RoundedPanel
...
$btnOpenExp.Add_Click({
    $dir = Join-Path $env:LOCALAPPDATA 'FpsTune\experiment'
...
$btnOpenBackup.Add_Click({
    $dir = Join-Path $env:LOCALAPPDATA 'FpsTune\backup'
    if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
    Start-Process explorer.exe $dir
})
