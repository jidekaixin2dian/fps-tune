...
v1.6.2 提供两种界面模式：标题栏可切换紧凑中文控制台与经典概览，并记住选择。控制台检测页显示数字和细进度条；经典检测页保留随窗口缩放即时重绘的折线图。优化页采用等宽预设、分类筛选、项目详情与固定执行栏；手动调整会进入自定义模式。设置页按类别整理，性能采样在页面间共享，并更新应用图标与适配任务栏缩放的托盘图标。
