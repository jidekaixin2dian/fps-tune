...
### 2.1 版本与同步状态核对（2026-09-25 实测，0.1.7 轮更新）
...
- **卡顿定位**：显示页**每次**进入（`Loaded` 每次触发）都在 UI 线程同步跑 DLSS / 数字振动 /
  ...
  **不虚拟化**（`IsVirtualizingWhenGrouping` 默认 false），每次筛选刷新 33 个富模板全部重实例化。
- **修复（`718a1be`）**：`DisplayQualityView.Refresh()` 改**快照机制**——免读取的部分
  （驱动建议、手动清单）立即画；四组状态读取进 `ComputeSnapshot`（线程池、逐项 try/catch 成
  错误文案），`ApplySnapshot` 回 UI 线程统一更新；gen 计数丢弃过期快照；`_busy` 守卫保证
  与写入动作不重叠。`OptimizeView.ItemList` 加
  `IsVirtualizingWhenGrouping=True` + `VirtualizationMode=Recycling`。
- **样式（`5e2d09e`）**：用户截图里的 DRS 卡 7 个下拉框是系统默认皮——其余页面的 ComboBox
  都挂 `SelectComboBoxStyle`，唯独这张卡漏了。补上即可，未引入新样式。
- **验证**：301/301；UIA 量化（点击→元素可见 ms）：优化页首切 186 / 二切 44，
  显示页首切 109 / 二切 89（**对照修复前 0.1.6 构建**：143/40、93/88——本机服务读取本就偏快，
  显性收益在读取慢的机器与优化页筛选点击；结构性保证 UI 线程零同步 IO）；
  异步回填验证（游戏行/DRS 值/振动百分比/ICC 当前 profile 全部出现）；DRS 卡截图目检
  （下拉框与全应用视觉一致）；`error.log` 全程不变。
- **0.1.7 候选与部署**：bump（`fab34e7`）→ 三资产哈希与清单逐项一致 → 部署 `D:\FpsTune`
  （备份 `-20260925-fab3`）→ 启动 GUI 给用户（15 秒存活、error.log 不变）。
- **push（用户指示"这轮改动做完可以 push 了"）**：`main` 已推 `origin/main`，本地与远端同步。
  **发 Release 仍等拍板。**

**⑧ 启动画面 + 启动预热 + 「我的方案」弹窗化（上一轮）**
