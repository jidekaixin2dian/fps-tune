...
7. **发布流程**：`做好 → git commit → 产出发布候选构建（publish-release.ps1，只写 dist/）
   → 部署到 D:\FpsTune → 启动 GUI 给用户看 → 等用户确认 → git push → gh release create`。
   **唯一权威版本在 `AGENTS.md` 硬纪律第 7 条**（含"哪些动作必须等拍板"的完整表格），动手前先读那一份。
   要点：拍板前**禁止** `git push` 与 `gh release create`；但 **`git commit`、`publish-release.ps1`
   构建、以及"部署到本机给用户看"都是允许的**——后者正是"把效果给用户看"的手段，
   与"未拍板不发版"不冲突。**构建脚本只写 `dist/`，不联网、不上传。**
...
| `docs/archive/REVIEW-1.6.1.md` | 上一轮公开审查的范围、修复项与**已知边界**（其中"不隔离已控制本机账户的攻击者"是本项目的既有声明，**至今有效**；其余为 1.x 存档） |
| `docs/README.md` | 文档总索引：现状文档 vs 历史存档，以及"先读什么" |
