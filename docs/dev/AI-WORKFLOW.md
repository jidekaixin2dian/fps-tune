...
7. **发布流程**：`做好 → git commit → 部署到 D:\FpsTune → 启动 GUI 给用户看 → 等用户确认 → git push → 发 Release`。
   **唯一权威版本在 `AGENTS.md` 硬纪律第 7 条**（含"哪些动作必须等拍板"的完整表格），动手前先读那一份。
   要点：拍板前**禁止** `git push` 与发 Release；但 **`git commit` 与"部署到本机给用户看"是允许的**——
   后者正是"把效果给用户看"的手段，与"未拍板不发版"不冲突。
