...
你是一个资深软件工程师，负责继续开发 fps-tune「FPS 帧律」。
...
- WPF 项目：`C:\Users\Aether\Documents\dsh\work\delta-skill\FpsTune.Wpf\`
...
2. 安装包：Inno Setup 6 已安装，`FpsTune-Setup-1.0.0.exe` 已构建成功。
...
- 安装 `FpsTune-Setup-1.0.0.exe` 并完成一次 Detect / Apply / Restore 真实往返。
