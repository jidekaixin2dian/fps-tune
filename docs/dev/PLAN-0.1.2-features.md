# 0.1.x 功能开发计划 · 显示与画质（N 卡驱动设置 / DLSS / ICC 滤镜）

> 状态（2026-09-25 校准）：**M1 / M2 / M3 全部完成**，已随 `v0.1.3-beta` 发布。
> - **M1** = DLSS 预设覆盖（真机闭环）
> - **M2** = ICC 滤镜（`IccFilterService` / `IccProfileGenerator` / `IccSystemApi`）
> - **M3** = DRS 二期：纹理过滤 / 电源管理 / 低延迟 / 透明度 AA
>
> ⚠️ **注意**：数字振动是**显示级 DVC**（`NvAPI_Get/SetDVCLevelEx`），**不是 DRS 项**——
> 官方 DRS 设置表里没有它。早期计划里"DRS 数字振动"的表述不准，已更正。
>
> 里程碑明细见 `docs/HANDOFF.md` §3（现状以那里为准，本文只保留开工依据与设计细节）。
> 前置阅读：`../ROADMAP.md`（五条红线）、`../../AGENTS.md`（代理协作纪律）、`../HANDOFF.md`（仓库现状）
> 当前基线：**266/266 测试**，.NET 10，已发布 `v0.1.3-beta`，本机安装位 `D:\FpsTune` 同版本。
> **发版流程铁律（2026-09-22）：本地构建 → git 提交 → 给用户看效果 → 用户拍板 → 才发 GitHub Release。**
