...
7. **发布流程（用户 2026-09-25 明确；本文是唯一权威版本，其它文档一律指向这里）**：
   ...
   | 3 | **部署到本机安装位**：把本次构建覆盖到 `D:\FpsTune`（整目录覆盖，**不跑安装器、不碰注册表**） | — |
   | 4 | **启动 GUI 给用户看**：让用户在真机上看效果 | — |
   | 5 | **等用户明确确认**（用户说"可以"之前，禁止下一步） | ← 等 |
   | 6 | **`git push`** | ← 拍板后 |
   | 7 | **发 Release**（`publish-release.ps1` / `build-installer.ps1` / `gh release create`） | ← 拍板后 |

   - **第 3–4 步属于"拍板前允许"的动作**：部署到 `D:\FpsTune` 并启动，就是"把效果给用户看"的手段。
     这与"未拍板不得发版"**不冲突**——禁止的是 push 与发 Release，不是本机部署。
   - 若这次改动不值得发版，第 3–4 步可跳过，改为只给**产物路径 + 关键改动摘要**。
   - **拍板前绝对禁止**：`git push`、上传 GitHub Release、跑发布脚本。
     **`git commit` 不受此限**——已验证的东西不允许留在工作区。
   - 部署前若 `D:\FpsTune` 里有进程在跑，先确认没有文件占用；覆盖失败要如实报告，不要静默跳过。
   - 用户如明确说"不用备份"，就按整目录覆盖做；否则先备份为 `D:\FpsTune-backup-YYYYMMDD`。
